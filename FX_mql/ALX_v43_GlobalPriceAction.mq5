...
void OnTick() {  
    ...
    if(lastTime != Time[0]) {
        ...
        if(isMarginEnough() == true || isMarginEnough() == false) {
...
void createBuy(string symbol, double sl) {
    ...
    double stoploss = request.price - 50*MarketInfo(symbol, MODE_POINT);
...
bool closeTrade(int i) {
    ...
    string symbol_s;
    StringConcatenate(symbol_s, trades[i].symbol, suffix);
    request.symbol = symbol_s;
