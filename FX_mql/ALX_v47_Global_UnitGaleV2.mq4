...
//---
    ...
    //----------------------------------------------------------------------- check new cycles of current trades
    checkTradeCycles();
    ...
    for(int i = 0; i < tradeIndex; i++) {
        ...
        for(int j = 0; j < trades[i].cycleIndex; j++) {
            if(trades[i].orders[j].op_type == OP_BUY) {  //CASE BUY
                ...
                Print("cycle ", j, " is at: ", points * trades[i].orders[j].size);
        ...
        Print("total current cycles at: ", accum);
        if(accum >= PIP_TARGET*INIT_SIZE  || accum <= -1*0.01*AccountBalance()) {
            closeCycles(i);
            reorderTrades(i);
        }   
    }
