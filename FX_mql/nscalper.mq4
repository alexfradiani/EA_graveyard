...
void OnTick() {
    ...
    if(lastTime != Time[1]) {
        ...
        last = Close[1];
        shortRSI = iRSI(NULL,0,5,PRICE_CLOSE,0);
        ...
        if(lows == 3) { //go short
			...
			int order = OrderSend(
				...
				oprice, //price
				5, //slippage???
				oprice - 50*Point,//NormalizeDouble(oprice-50,5), //Stop loss
				0 //Take profit
			);
        ...
        if(highs == 3 && shortRSI>65 && shortRSI<80 && ma2>=MarketInfo(Symbol(),MODE_BID)) {  //go long
        ...
        for(int i = 0; i < ArraySize(shorts); i++) {
            ...
            if(orderWillClose) {
                double oprice = MarketInfo(Symbol(), MODE_BID);
                OrderClose(shorts[i].ticket, risk_vol, oprice, 3);
        ...
        for(int i = 0; i < ArraySize(longs); i++) {
            ...
            if(orderWillClose) {
                double oprice = MarketInfo(Symbol(), MODE_ASK);
                OrderClose(longs[i].ticket, risk_vol, oprice, 3);
