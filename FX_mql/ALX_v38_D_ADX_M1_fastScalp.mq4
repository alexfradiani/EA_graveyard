...
void OnTick() { 
    //------------------------------------------------------------------------- TRADE rules
    //ADX D1 global trend
    double adxMinus = iADX(NULL, PERIOD_D1, 7, PRICE_CLOSE, MODE_MINUSDI, 0);
    double adxPlus = iADX(NULL, PERIOD_D1, 7, PRICE_CLOSE, MODE_PLUSDI, 0);
    ...
    double ma_0 = iMA(NULL, PERIOD_M1, 3, 0, MODE_SMA, PRICE_TYPICAL, 0);
    double ma_1 = iMA(NULL, PERIOD_M1, 3, 0, MODE_SMA, PRICE_TYPICAL, 1);
    
    double acc = (ma_0 - ma_1)/Point;
    
    if(acc < 0 && movement == UP) {
        Print("ACC DOWN: ", acc);
        movement = DOWN;
    ...
    if(buyOrder.ticket != -1) {
        ...
        if(Bid < buyOrder.price) {
            if(buyOrder.price - Bid > atr)
                closeBuy();
        }
        else {
    ...
    if(sellOrder.ticket != -1) {
        ...
        if(Ask > sellOrder.price) {
            if(Ask - sellOrder.price > atr)
                closeSell();
        }
        else {
            if(sellOrder.price - Ask >= atr)
                if(Ask > ma_0)
                    closeSell();
        }
    }
