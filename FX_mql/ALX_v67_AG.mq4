...
int OnInit() {
    ...
    lastBar = Time[0];
    GRID_Y = 117;//iATR(Symbol(), PERIOD_M30, 300, 0)/MarketInfo(Symbol(), MODE_POINT);
...
void OnTick() {
    ...
    if(systemsActive() == TRUE) { //current cycle in process
        ...
        if(AG[AG_i].op_type == OP_BUY) {
            ...
            else if(ask - AG[AG_i].price >= GRID_Y * point) {
                AG_createBuy();
            }
        }
            ...
            else if(AG[AG_i].price - bid >= GRID_Y * point) {
                AG_createSell();
            }
        }
            ...
            if(AG[i].ticket != -1) {
                if(AG[i].op_type == OP_BUY) {
                    if(bid - AG[i].price <= -1 * GRID_Y * point)
                        AG_close(i);
                    else if(ask - AG[i].price >= TARGET * point)
                        AG_close(i);
                }
                else if(AG[i].op_type == OP_SELL) {
                    if(AG[i].price - ask <= -1 * GRID_Y * point)
                        AG_close(i);
                    else if(AG[i].price - bid >= TARGET * point)
                        AG_close(i);
                }
            }
        }
