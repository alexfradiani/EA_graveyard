...
input bool CloseAllSession = true; // Close all trades after session(s)
...
input ENUM_TIMEFRAMES TimeFrame  = 60; //TimeFrame to open chart
...
bool CloseAll;
string postfix=StringSubstr(Symbol(),6,2);
int   symb_cnt=0;
...
//SetPanel("Panel"+IntegerToString(i),0,x_axis,(i*16)+y_axis,68,15,labelcolor,LineColor,1);         
         ...
         SetPanel("m1"+IntegerToString(i),0,x_axis+180,(i*16)+y_axis,20,15,Color,White,1);
...
//         ColorPanel("BP",C'30,30,30',C'61,61,61',1);
         ...
         ColorPanel("m1"+IntegerToString(i),Color,White);
        ...
        // ColorPanel("tc"+IntegerToString(i),labelcolor,C'61,61,61');
...
bool inSession() {
   ...
   s1end = StrToTime(sess1end);
   s2start = StrToTime(sess2start);
   s2end = StrToTime(sess2end);
   s3start = StrToTime(sess3start);
   s3end = StrToTime(sess3end);

   if (StopProfit != 0 && profitbaskets >= StopProfit)
      return(false);
