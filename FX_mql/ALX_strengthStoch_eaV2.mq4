...
void evaluateStochs(string pair) {
    ...
    if(m5.lastTrigger == UP && m5.climbing == UP) { //conditions for longs
        if(m15.lastTrigger == UP && m15.climbing >= NONE && m15.weakClimb == UP) { 
            if(m30.lastTrigger == UP && m30.climbing >= NONE && m30.weakClimb == UP) {
    ...
    if(m5.lastTrigger == DOWN && m5.climbing == DOWN) { //conditions for shorts
        if(m15.lastTrigger == DOWN && m15.climbing <= NONE && m15.weakClimb == DOWN) {
            if(m30.lastTrigger == DOWN && m30.climbing <= NONE && m30.weakClimb == DOWN) {
    ...
    if(sellOrder.ticket != -1) {
        if(Ask >= sellOrder.sl) { //critic stoploss
            ...
            //check if SL can be changed
            if(sellOrder.sl < sellOrder.price) { //already stepping
                if(sellOrder.sl - Ask >= STEP_PTS*Point - 5*Point)
                    sellOrder.sl = Ask + 5*Point;
            }
            else {
                if(sellOrder.price - Ask >= STEP_PTS*Point - 5*Point) //start step sl
                    sellOrder.sl = Ask + 5*Point;
            }
        }
    }
