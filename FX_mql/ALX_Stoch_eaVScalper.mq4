...
void evaluateStochs() {
    ...
    if(Time[0] != lastTime) {  //Check which stochastics can be updated
        ...
        if(m5_st <= 11) {
            ...
            if(m5.lastTrigger == UP && priceCorrelation(UP, pair) == UP)
                if(m30.lastTrigger == UP && m30.weakClimb == UP)
                    if(buyOrder.ticket == -1)
                        createBuy(pair);
            
            if(m5.lastTrigger == DOWN && priceCorrelation(DOWN, pair) == DOWN)
...
void createBuy(string pair) {
	...
	double stoploss = oprice - STEP_PT*Point;
	double takeprofit = oprice + STEP_PT*Point;
	int order = OrderSend(
		pair, //symbol
		optype, //operation
		R_VOL, //volume
