...
void evaluateStochs() {
    ...
    if(buyOrder.ticket != -1) { 
        ...
        if(m15.lastTrigger == DOWN || Bid - buyOrder.price >= 100*Point) {
            if(Bid - 30*Point > buyOrder.sl)
                buyOrder.sl = Bid - 30*Point;
        }
    }
    ...
    if(sellOrder.ticket != -1) {
        ...
        if(m15.lastTrigger == UP || sellOrder.price - Ask >= 100*Point) {
            if(Ask + 30*Point < sellOrder.sl)
                sellOrder.sl = Ask + 30*Point;
        }
    }
}
