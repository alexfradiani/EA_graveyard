...
#define DI_MIN_DISTANCE 1.0

#define UP 1
#define DOWN -1
#define NONE 0

#define BASE_PIPS 1000
...
void OnTick() { 
    ...
    if(Time[0] != lastBarTime) {
        ...
        lastBarTime = Time[0];
        Print("trendState: ", trendState);
    }
    
    /*if(buyOrder.ticket != -1) {
        if(Bid < buyOrder.sl)
            closeBuy();
        else if(Bid - buyOrder.price > 2*BASE_PIPS*Point)
            closeBuy();
        
        if(Bid - buyOrder.price >= 100*Point)
            buyOrder.sl = buyOrder.price + 10*Point;
    }
    
    if(sellOrder.ticket != -1) {
        if(Ask > sellOrder.sl)
            closeSell();
        else if(sellOrder.price - Ask > 2*BASE_PIPS*Point)
            closeSell();
            
        if(sellOrder.price - Ask >= 100*Point)
            sellOrder.sl = sellOrder.price - 10*Point;
    }*/
