 ...
 * This prototype is for backtesting. Real system will be divided in SideA and SideB
 *
 ...
 *
 * Full cycle closes when both sides profit is >= 0 (equilibrium point) OR Max loss taken  
 */
...
void OnTick() {
    ...
    if(cycleIsActive == TRUE) {
        if(bid <= A_order.sl) {  //new level in buy side
            A_createBuy(A_order.size + SIZE_STEP);
            B_tryNewLevel();
        }
        
        if(ask >= B_order.sl) {  //new level in sell side
        ...
        if(accumA > 0 && accumB > 0) {
            Print("accumA: ", accumA, " accumB: ", accumB);
            closeCycle();
        }
        else if(accumA + accumB <= -1 * MAX_LOSS) {  //LOSS mgmt
            closeCycle();
        }
    }
