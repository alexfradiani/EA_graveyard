...
//---
    ...
    double hma = iMA(Symbol(), PERIOD_M1, 180, 0, MODE_SMA, PRICE_CLOSE, 0);
    
    double m3 = iMA(Symbol(), PERIOD_M1, 3, 0, MODE_SMA, PRICE_CLOSE, 0);
    
    double point = MarketInfo(Symbol(), MODE_POINT);
    if(lastTime != Time[0]) {
        int prevPriceMovH = priceMovH;
        ...
        if(prevPriceMovH < priceMovH && rootBuyCycle.followsCount == 0)
            createRootBuy();
        if(prevPriceMov8H < priceMov8H && rootBuyCycle.followsCount == 0)
            createRootBuy();
            
        if(prevPriceMovH > priceMovH && rootSellCycle.followsCount == 0)
            createRootSell();
        if(prevPriceMov8H > priceMov8H && rootSellCycle.followsCount == 0)
            createRootSell();
          
        //------------------------------------------------------------------------------ trigger of peak orders
        if(prevPriceMovH == priceMovH && priceMovH == UP)
        ...
        if(prevPriceMovH == priceMovH && priceMovH == DOWN)
...
void createRootBuy() {
    ...
    if(rootBuyCycle.followsCount > 0) {
        ...
        osize = rootBuyCycle.currFollow.size * 3;  
    }
